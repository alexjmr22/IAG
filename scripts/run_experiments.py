...
EXPERIMENTS = {
    ...
    '3': [ # PC 3 — Diffusion COMPLETE Battery
        # --- Basic Sweeps (Steps / Channels / LR) ---
...
def main():
    ...
    parser.add_argument('--pc', type=str, required=True, choices=['1', '2', '3'], help="ID do Computador (1=VAE, 2=GAN, 3=DIFF)")
    ...
    for exp in pc_experiments:
        ...
        if target == 'VAE':
            ...
            run_script(root_dir / 'scripts' / script_map[target], exp_env)
            if target == 'WGAN': exp_env['EVAL_TARGET'] = 'DCGAN'
        elif target == 'Diffusion' or target == 'DiffusionEMA':
            script_name = '03_diffusion.py' if target == 'Diffusion' else '03b_diffusion_ema.py'
            run_script(root_dir / 'scripts' / script_name, exp_env)
